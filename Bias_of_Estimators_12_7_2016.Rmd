...
```{r data_setup_1000, include= FALSE}
...
sub.1 =YesYes_1000 %>% filter(True.CV == sub$True.CV)
```
...
```{r pwr_graph, echo = FALSE, fig.cap="Power Versus the True $\\omega_3$ value for simulations. Grouped by the CV of SRV and the method used to analyze the replicates."}
a.pwr.tab = pwr_tab(sub.1)
cl = colors()
...
```{r tile_graph, echo = FALSE, fig.pos='H',fig.cap= "Showing how the power changes for both analysis as the amount of positive selection and CV of SRV inxrease. Each square represents 100 replicates simulated with the given values.The color corespond to the power analysis to correctly identify positive selection."}
...
omega1.bias = sub.1 %>% group_by(True.omega3.value, True.CV) %>% summarise_at(vars(matches("omega1")), funs(mean,median))
...
d  %>%  ggplot(aes(x = True.omega3.value, y = value,color = variable )) +
  geom_point() + geom_smooth()+ facet_grid(True.CV~analysis) + labs(x = expression("True "*omega[3]), y = expression(omega[1]))+scale_color_discrete(name = "Measure", breaks = c("mean.MLE", "True.value.Mean", "median.MLE"), 
                                                        labels = c("Mean MLE", expression("True "*omega[1]), "Median MLE"))
```

```{r graph_prop_omega1_1000, echo =FALSE, fig.cap="Bias of $\\omega_1$ proportion. The green line is the proportion of sites that were simulated with the true $\\omega_1$ value. The orange line is the mean proportion of sites with $\\omega_1$, while the blue line is the median, according to the respective analysis. "}
d = c %>% select(True.omega3.value,True.CV, analysis,mean.prop, True.Prop.Mean, median.prop) %>%
  melt(id.vars = c("True.omega3.value","True.CV", "analysis"))

d  %>%  ggplot(aes(x = True.omega3.value, y = value,color = variable )) +
  geom_point() + geom_smooth()+ facet_grid(True.CV~analysis) +labs(x = expression("True "*omega[3]), y = expression(omega[1]*" prop"))+ scale_color_discrete(name = "Measure", breaks = c("mean.prop", "True.Prop.Mean", "median.prop"), labels = c("Mean prop", expression("True "*omega[1]*" prop"), "Median prop"))
...
omega2.bias = sub.1 %>% group_by(True.omega3.value, True.CV) %>% summarise_at(vars(matches("omega2")), funs(mean,median))
...
d  %>%  ggplot(aes(x = True.omega3.value, y = value,color = variable )) +
  ...
  labs(x = expression("True "*omega[3]), y = expression(omega[2]))+
...
```{r graph prop omega2 1000, echo=FALSE, fig.cap="The mean and median proportion of sites with $\\omega_2$  versus the true proportion of sites simulated to have that value."}
d = c %>% select(True.omega3.value,True.CV, analysis,mean.prop, True.Prop.Mean, median.prop) %>%
  melt(id.vars = c("True.omega3.value","True.CV", "analysis"))

d  %>%  ggplot(aes(x = True.omega3.value, y = value,color = variable )) +
  geom_point() + geom_smooth()+ facet_grid(True.CV~analysis)+labs(x = expression("True "*omega[3]), y = expression(omega[2]))+ scale_color_discrete(name = "Measure", breaks = c("mean.prop", "True.Prop.Mean", "median.prop"), labels = c("Mean prop", expression("True "*omega[2]*" prop"), "Median prop"))
...
```{r table for omega3 1000, message=FALSE, warning=FALSE, echo=FALSE, results="asis"}

omega3.bias = sub.1 %>% group_by(True.omega3.value, True.CV) %>% summarise_at(vars(matches("omega3")), funs(mean,median))
...
d  %>%  ggplot(aes(x = True.omega3.value, y = value,color = variable )) +
  geom_point() + geom_smooth()+ facet_grid(True.CV~analysis)+
  labs(x = expression("True "*omega[3]), y = expression(omega[3]*" prop"))+
...
alpha1.bias = sub.1 %>% group_by(True.omega3.value, True.CV) %>% summarise_at(vars(matches("alpha1")), funs(mean,median))
...
d  %>%  ggplot(aes(x = True.omega3.value, y = value,color = variable )) +
  geom_point() + geom_smooth()+ facet_grid(~True.CV) + 
  labs(x = expression("True "*omega[3]), y = expression(alpha[1]))+
  scale_color_discrete(name = "Measure", breaks = c("mean.MLE", "True.value.Mean", "median.MLE"), labels = c("Mean MLE", expression("True "*alpha[1]), "Median MLE")) + coord_cartesian(ylim = c(0,1.5))
```
...
d  %>%  ggplot(aes(x = True.omega3.value, y = value,color = variable )) +
  geom_point() + geom_smooth()+ facet_grid(~True.CV)+
  labs(x = expression("True "*omega[3]), y = expression(alpha[1]*" prop"))+
...
alpha2.bias = sub.1 %>% group_by(True.omega3.value, True.CV) %>% summarise_at(vars(matches("alpha2")), funs(mean,median))
...
d  %>%  ggplot(aes(x = True.omega3.value, y = value,color = variable )) +
  geom_point() + geom_smooth()+ facet_grid(~True.CV)+ 
  labs(x = expression("True "*omega[3]), y = expression(alpha[2]))+
  scale_color_discrete(name = "Measure", breaks = c("mean.MLE", "True.value.Mean", "median.MLE"), labels = c("Mean MLE", expression("True "*alpha[2]), "Median MLE"))+coord_cartesian(ylim = c(0,3))
```
...
d  %>%  ggplot(aes(x = True.omega3.value, y = value,color = variable )) +
  geom_point() + geom_smooth()+ facet_grid(~True.CV)+
  labs(x = expression("True "*omega[3]), y = expression(alpha[2]*" prop"))+
...
alpha3.bias = sub.1 %>% group_by(True.omega3.value, True.CV) %>% summarise_at(vars(matches("alpha3")), funs(mean,median))
...
d  %>%  ggplot(aes(x = True.omega3.value, y = value,color = variable )) +
  geom_point() + geom_smooth()+ facet_grid(~True.CV)+ 
  labs(x = expression("True "*omega[3]), y = expression(alpha[3]))+
  scale_color_discrete(name = "Measure", breaks = c("mean.MLE", "True.value.Mean", "median.MLE"), labels = c("Mean MLE", expression("True "*alpha[3]), "Median MLE"))+coord_cartesian(ylim = c(0,8))
```
...
d  %>%  ggplot(aes(x = True.omega3.value, y = value,color = variable )) +
  geom_point() + geom_smooth()+ facet_grid(True.CV~analysis)+
  labs(x = expression("True "*omega[3]), y = expression(alpha[3]*" prop"))+
...
CVs = sub.1 %>% select(True.omega3.value, True.CV, CV.SRV) %>% group_by(True.omega3.value, True.CV) %>% summarise_at(vars(matches("CV.SRV")), funs(mean,median))
...
```{r branch_length_data, echo = FALSE}
srv.T_scaler = 4
srv.tree.Node1.t=0
...
br.len =idk %>% filter(True.CV %in% sub$True.CV)
br.len$True.CV = round(br.len$True.CV, 3)
```

```{r branch_1, echo=FALSE}
#now we're just taking branch 1 and getting the mean and median length as grouped by true omega3 and true CV

br.1 = br.len %>% filter(Branch == 1)%>% group_by(True.omega3.value, True.CV) %>% summarise_at(vars(matches("length")), funs(mean, median))
...
```{r branch_2, echo=FALSE}
#now we're just taking branch 1 and getting the mean and median length as grouped by true omega3 and true CV

br.2 = a %>% filter(Branch == 2)%>% group_by(True.omega3.value, True.CV) %>% summarise_at(vars(matches("length")), funs(mean, median))

#melting things to allow us to color and facet as we need
t = br.2 %>% melt(id.vars = c("True.omega3.value", "True.CV")) %>% mutate(method = str_extract(variable,"BUSTED|SRV|truth"), 
                                                               measure = str_extract(variable, "mean|median|truth")
)
...
```{r branch_3, echo=FALSE}
#now we're just taking branch 1 and getting the mean and median length as grouped by true omega3 and true CV

br.3 = a %>% filter(Branch == 3)%>% group_by(True.omega3.value, True.CV) %>% summarise_at(vars(matches("length")), funs(mean, median))

#melting things to allow us to color and facet as we need
t = br.3 %>% melt(id.vars = c("True.omega3.value", "True.CV")) %>% mutate(method = str_extract(variable,"BUSTED|SRV|truth"), 
                                                               measure = str_extract(variable, "mean|median|truth")
)
...
```{r branch_4, echo=FALSE}
#now we're just taking branch 1 and getting the mean and median length as grouped by true omega3 and true CV

br.4 = a %>% filter(Branch == 4)%>% group_by(True.omega3.value, True.CV) %>% summarise_at(vars(matches("length")), funs(mean, median))

#melting things to allow us to color and facet as we need
t = br.4 %>% melt(id.vars = c("True.omega3.value", "True.CV")) %>% mutate(method = str_extract(variable,"BUSTED|SRV|truth"), 
                                                               measure = str_extract(variable, "mean|median|truth")
)
...
#red hline is the true value for branch 1 according to what I pulled out of the LF 
t %>% ggplot(aes(x = True.omega3.value, y = value, color = measure))+ geom_point()+ facet_grid(True.CV~method)+
  geom_hline(yintercept = srv.tree.4.t, color = 'red')

```

#Graphs for 10,000 Codons

```{r 10000 data setup, echo = FALSE}
...
```{r 10000 omega1 prop, echo=FALSE}
d = c %>% select(True.omega3.value,True.CV, analysis,mean.prop, True.Prop.Mean, median.prop) %>%
  melt(id.vars = c("True.omega3.value","True.CV", "analysis"))
...
omega1.bias = sub.10 %>% group_by(True.omega3.value, True.CV) %>% summarise_at(vars(matches("omega2")), funs(mean,median))

b= omega1.bias %>% gather(key = test, value = estimate, - True.omega3.value, -True.CV, -True.omega2.value_mean, -True.omega2.prop_mean, -True.omega2.value_median, -True.omega2.prop_median) 
...
d  %>%  ggplot(aes(x = True.omega3.value, y = value,color = variable )) +
  geom_point() + geom_smooth()+ facet_grid(True.CV~analysis) + labs(x = expression("True "*omega[3]), y = expression(omega[2]))+scale_color_discrete(name = "Measure", breaks = c("mean.MLE", "True.value.Mean", "median.MLE"), 
                                                        labels = c("Mean MLE", expression("True "*omega[2]), "Median MLE"))
```


```{r 10000 omega2 prop, echo=FALSE}
d = c %>% select(True.omega3.value,True.CV, analysis,mean.prop, True.Prop.Mean, median.prop) %>%
  melt(id.vars = c("True.omega3.value","True.CV", "analysis"))

d  %>%  ggplot(aes(x = True.omega3.value, y = value,color = variable )) +
  geom_point() + geom_smooth()+ facet_grid(True.CV~analysis) +labs(x = expression("True "*omega[3]), y = expression(omega[2]*" prop"))+ scale_color_discrete(name = "Measure", breaks = c("mean.prop", "True.Prop.Mean", "median.prop"), labels = c("Mean prop", expression("True "*omega[2]*" prop"), "Median prop"))
```
...
omega3.bias = sub.10 %>% group_by(True.omega3.value, True.CV) %>% summarise_at(vars(matches("omega3")), funs(mean,median))
...
alpha1.bias = sub.10 %>% group_by(True.omega3.value, True.CV) %>% summarise_at(vars(matches("alpha1")), funs(mean,median))
...
```{r , message=FALSE, warning=FALSE, echo=FALSE, results="asis"}
alpha2.bias = sub.10 %>% group_by(True.omega3.value, True.CV) %>% summarise_at(vars(matches("alpha2")), funs(mean,median))
...
alpha3.bias = sub.10 %>% group_by(True.omega3.value, True.CV) %>% summarise_at(vars(matches("alpha3")), funs(mean,median))
...
omega1.bias = sub.6 %>% group_by(True.omega3.value, True.CV) %>% summarise_at(vars(matches("omega1")), funs(mean,median))
...
omega1.bias = sub.6 %>% group_by(True.omega3.value, True.CV) %>% summarise_at(vars(matches("omega2")), funs(mean,median))
...
omega3.bias = sub.6 %>% group_by(True.omega3.value, True.CV) %>% summarise_at(vars(matches("omega3")), funs(mean,median))
...
alpha1.bias = sub.6 %>% group_by(True.omega3.value, True.CV) %>% summarise_at(vars(matches("alpha1")), funs(mean,median))
...
```{r , message=FALSE, warning=FALSE, echo=FALSE, results="asis"}
alpha2.bias = sub.6 %>% group_by(True.omega3.value, True.CV) %>% summarise_at(vars(matches("alpha2")), funs(mean,median))
...
alpha3.bias = sub.6 %>% group_by(True.omega3.value, True.CV) %>% summarise_at(vars(matches("alpha3")), funs(mean,median))
...
b %>%  filter( stat == "MLE") %>% ggplot(aes(x =True.omega3.value, y = estimate, color = factor(Sites))) + geom_point() + geom_smooth(se =F)+
  facet_grid(True.CV ~ analysis) + geom_hline(yintercept = b$True.omega1.value, color = "red", aes(show.legend = TRUE)) + labs(x = expression("True "*omega[3]), y = expression(omega[1]))
```
...
```{r, echo = F}
#just using the median as our stat
omega2.bias = all.sub %>% group_by(True.omega3.value, True.CV, Sites) %>% summarise_at(vars(matches("omega2")), median)

b= omega2.bias %>% gather(key = test, value = estimate, - True.omega3.value, -True.CV, -Sites, -True.omega2.value, -True.omega2.prop) 
...
```{r, echo = FALSE}
b %>%  filter( stat == "MLE") %>% ggplot(aes(x =True.omega3.value, y = estimate, color = factor(Sites))) + geom_point() + geom_smooth(se =F)+
  facet_grid(True.CV ~ analysis) + geom_hline(yintercept = b$True.omega2.value, color = "red", aes(show.legend = TRUE))+ labs(x = expression("True "*omega[3]), y = expression(omega[2]))
```
...
```{r, echo = F}
#just using the median as our stat
omega3.bias = all.sub %>% group_by(True.omega3.value, True.CV, Sites) %>% summarise_at(vars(matches("omega3")), median)

b= omega3.bias %>% gather(key = test, value = estimate, - True.omega3.value, -True.CV, -Sites, -True.omega3.prop) 

b = mutate(ungroup(b), stat = str_extract(b$test, "MLE|prop"), analysis = str_extract(b$test,"BUSTED.SRV\\.|BUSTED\\.|True"))
```

```{r, echo = FALSE}
b %>%  filter( stat == "MLE") %>% ggplot(aes(x =True.omega3.value, y = estimate, color = factor(Sites))) + geom_point() + geom_smooth(se =F) +  facet_grid(True.CV ~ analysis) + geom_abline(slope = 1, intercept = 0, color = 'red') + labs(x = expression("True "*omega[3]), y = expression(omega[3]))
```
...
```{r, echo = F}
#just using the median as our stat
alpha1.bias = all.sub %>% group_by(True.omega3.value, True.CV, Sites) %>% summarise_at(vars(matches("alpha1")), median)

b= alpha1.bias %>% gather(key = test, value = estimate, - True.omega3.value, -True.CV, -Sites, -True.alpha1.value, -True.alpha1.prop) 

b = mutate(ungroup(b), stat = str_extract(b$test, "MLE|prop"), analysis = str_extract(b$test,"BUSTED.SRV\\.|BUSTED\\.|True"))
```

```{r, echo = FALSE}
b %>%  filter( stat == "MLE") %>% ggplot(aes(x =True.omega3.value, y = estimate, color = factor(Sites))) + geom_point() + geom_smooth(se =F)+ geom_hline(aes(yintercept = True.alpha1.value), color = "red", show.legend = TRUE)+
  facet_grid(~True.CV) 
```
...
```{r, echo = F}
#just using the median as our stat
alpha2.bias = all.sub %>% group_by(True.omega3.value, True.CV, Sites) %>% summarise_at(vars(matches("alpha2")), median)

b= alpha2.bias %>% gather(key = test, value = estimate, - True.omega3.value, -True.CV, -Sites, -True.alpha2.value, -True.alpha2.prop) 
...
```{r, echo = FALSE}
b %>%  filter( stat == "MLE") %>% ggplot(aes(x =True.omega3.value, y = estimate, color = factor(Sites))) + geom_point() + geom_smooth(se =F)+
  facet_grid(~True.CV ) + geom_hline(aes(yintercept = True.alpha2.value), color = "red", show.legend = TRUE)
```


```{r, echo = F}
#just using the median as our stat
alpha3.bias = all.sub %>% group_by(True.omega3.value, True.CV, Sites) %>% summarise_at(vars(matches("alpha3")), median)

b= alpha3.bias %>% gather(key = test, value = estimate, - True.omega3.value, -True.CV, -Sites, -True.alpha3.value, -True.alpha3.prop) 
...
```{r, echo = FALSE}
b %>%  filter( stat == "MLE") %>% ggplot(aes(x =True.omega3.value, y = estimate, color = factor(Sites))) + geom_point() + geom_smooth(se =F)+
  facet_grid(True.CV ~ analysis) + geom_hline(aes(yintercept = True.alpha3.value), color = "red", show.legend = TRUE)
```
